...
function getStatus(rec: NutrientRecord | null) {
  if (!rec) return "On-Going";
  const arr = parseNutrientData(rec.nutrient_data);
  if (arr.length === 0) return "On-Going";
  const flags = arr.map((r) => readingHasData(r));
  if (flags.every((f) => !f)) return "On-Going";
  if (flags.every((f) => f)) return "Completed";
}
...
export default function NutrientMonitoringTable() {
  ...
  cell: ({ row }) => maskName(row.original.farmer_name),
    ...
    columnHelper.accessor("nutrient_data", { header: "Nutrient Data" }),
    ...
    const rows = table.getFilteredRowModel().rows.map((r) =>
      visibleCols
        .map((col) => {
          ...
          if (col.id === "farmer_name") {
            return maskName(String(v));
          }
...
const blob = new Blob([BOM + csv], {
  ...
  const filledCount = data.filter((d) => getStatus(d) === "Completed").length;
  const partialCount = data.filter((d) => getStatus(d) === "On-Going").length;
