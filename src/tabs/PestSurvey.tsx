...
export default function PestObservationTable() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const res = await fetch("/api/pest_survey/dashboard/get_all_records", {
          headers: {
        ...
        json.forEach((farmer: FarmerRecord) => {
          try {
            const observations: PestObservation[] = JSON.parse(farmer.pest_survey);
            ...
            console.error("Failed to parse pest_survey for farmer:", farmer.farmer_name, e);
          }
        });
  ...
  async function handleExport() {
    ...
    try {
      const token = localStorage.getItem("authToken");
      const url = `/api/pest_survey/dashboard/get_export_data?season_id=${exportSeasonId}&season_year=${exportSeasonYear}`;
      ...
      a.href = url2;
      a.download = `pest_survey_${SEASONS.find(s => s.id === exportSeasonId)?.name}_${exportSeasonYear}.csv`;
      a.click();
  ...
  async function saveRecord() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/pest_survey/dashboard/update/${selected.observation_id}`,
        {
          method: "PUT",
          headers: {
          ...
          body: JSON.stringify({
            pest_name: editForm.pest_name || null,
      ...
      setIsEditing(false);
      alert("Successfully updated pest observation");
    } catch (error) {
      alert("Failed to update pest observation");
    } finally {
      setIsSaving(false);
    }
  }
              ...
              {isEditing ? (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="text-sm font-medium">Pest Name</label>
                      <input
                        type="text"
                        value={editForm.pest_name}
                        onChange={e => setEditForm({ ...editForm, pest_name: e.target.value })}
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <Field name="Pest Name" value={selected.pest_name} />
