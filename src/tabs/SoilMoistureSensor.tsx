...
// -----------------------------------------------------
// Modal Field Label Mapping (CHANGE)
// -----------------------------------------------------
const FIELD_LABELS: Partial<Record<keyof SoilLiveRecord, string>> = {
  sensor1_value: "Sensor1 Value (vol/vol)",
  sensor2_value: "Sensor2 Value (vol/vol)",
};
...
export default function SoilMoistureLiveTable() {
  ...
  const columns = [
    ...
    columnHelper.accessor("sensor1_value", { header: "Value 1 (vol/vol)" }),
    columnHelper.accessor("sensor2_value", { header: "Value 2 (vol/vol)" }),
        ...
        <table className={THEME.table.table}>
          ...
          <tbody>
            {table.getRowModel().rows.map((row, i) => (
              <tr
                key={row.id}
                className={`${i % 2 ? THEME.table.rowOdd : THEME.table.rowEven} ${THEME.table.rowHover}`}
              >
