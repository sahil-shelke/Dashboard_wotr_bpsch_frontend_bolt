...
import { Save, Users, FileEdit as Edit, Trash2, Plus, Filter } from 'lucide-react';
...
const RegionalManagers: React.FC = () => {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<RegionalManagerData>({
    defaultValues: {
      role_id: 2 // Regional Manager role
    }
  });
...
const onSubmit = async (data: RegionalManagerData) => {
  setLoading(true);
  try {
    const token = localStorage.getItem('token');
    const headers = {
  ...
  const handleDelete = async (phone_number: string) => {
    if (window.confirm('Are you sure you want to delete this Regional Manager?')) {
      try {
        ...
        toast.error('Failed to delete Regional Manager');
      }
    }
  };
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b">
              <h2 className="text-xl font-bold text-gray-900">
                {editingPhone ? 'Edit Regional Manager' : 'Add Regional Manager'}
              </h2>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <input type="hidden" {...register('role_id')} value={2} />
              </div>
        ...
        <div className="overflow-x-auto">
          ...
          {filteredManagers.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No Regional Managers found</h3>
              <p className="text-gray-600">
                {managers.length === 0 ? 'Get started by adding your first Regional Manager.' : 'Try adjusting your filters.'}
              </p>
            </div>
          )}
        </div>
